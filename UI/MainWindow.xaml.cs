...
namespace TestReporter
{
    public partial class MainWindow : Window
    {
        ...
        // Строит матричное представление: строки = (Тема, Студент), столбцы = Тема, Студент, Сумма баллов
        private void BuildMatrixPreview(IEnumerable<StudentAnswer> records)
        {
            try
            {
                ...
                table.Columns.Add("Сумма баллов", typeof(int));
                ...
                foreach (var g in groups.OrderBy(x => x.Key.TopicName).ThenBy(x => x.Key.StudentName))
                {
                    ...
                    int rowSum = g.Sum(x => x.Score ?? 0);
                    row["Сумма баллов"] = rowSum;
